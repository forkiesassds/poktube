    ...
    foreach ($comments as $comment) {
        if ($comment['parent_id'] == $parent_id) {
			...
			<div class="comment">
                ...
                <br>
                - <a href="profile.php?user=' . htmlspecialchars($comment['name'], ENT_QUOTES) . '">' . htmlspecialchars($comment['name'], ENT_QUOTES) . '</a> - (' . time_elapsed_string($comment['submit_date']) . ')
            </div>
            ';
        }
    }
...
function show_write_comment_form($parent_id = -1) {
    ...
    <div class="write_comment" data-comment-id="' . $parent_id . '">
        <form>
            ...
            <input name="name" type="hidden" readonly="readonly" value="' .  $_SESSION['username'] . '">
            <textarea cols="55" rows="3" name="content" placeholder="Write your comment here..." required></textarea><br>
...
<div class="comment_header">
        <div class="c_title">
            Comments (<?=$comments_info['total_comments']?>)
        </div>
</div>
